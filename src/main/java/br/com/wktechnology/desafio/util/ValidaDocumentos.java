...
public class ValidaDocumentos {
    ...
    public static String removeContaMask(String str) {
        ...
        str = str.replace(" ", ""); //tira espaço em branco
        str = str.replace(".", ""); //tira ponto
        str = str.replace("/", ""); //tira barra
        str = str.replace("-", ""); //tira hífen
    ...
    public static boolean isValidoCPF(String cpf) {
        ...
        String removeContaMask = removeContaMask(cpf);


        try {
            if ((removeContaMask == null) || (removeContaMask.length() != 11)) {
                return false;
            }
            Integer digito1 = calcularDigito(removeContaMask.substring(0, 9), pesoCPF);
            Integer digito2 = calcularDigito(removeContaMask.substring(0, 9) + digito1, pesoCPF);
            boolean d = cpf.equals(cpf.substring(0, 9) + digito1.toString() + digito2.toString());
            return removeContaMask.equals(removeContaMask.substring(0, 9) + digito1.toString() + digito2.toString());
        } catch (Exception e) {
            return false;
        }
    }
